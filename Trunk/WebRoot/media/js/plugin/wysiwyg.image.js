	...
	$.wysiwyg.controls.image = {
		...
		init: function (Wysiwyg) {
			...
			formImageHtml = '<form class="wysiwyg" id="wysiwyg-addImage"><fieldset>' +
				'<div class="form-row"><label for="name">{url}:</label><div class="form-row-value"><input type="hidden" id="src" name="src" value=""/><input type="file" id="fileName" name="fileName" value=""/>';
			...
			$(adialog).bind("afterOpen", function (e, dialog) {
				dialog.find("form#wysiwyg-addImage").submit(function (e) {
					...
					alert(Wysiwyg.controls.insertImage.uploadAction);
					var files = new Array();
					files.push($("#fileName").attr("name"));
					$.ajaxFileUpload({
						url:$.URL.get(Wysiwyg.controls.insertImage.uploadAction),
								//"/file/image/fault/upload.action"),
						secureuri :false,
						fileElementId :files,
						dataType:'json',
						data : data,
						...
						success : function (response, status){
							if(response.status_code == '0'){
								$('input[name="src"]', dialog.container).val(response.data.finalFileName);
